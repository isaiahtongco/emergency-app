...
const MonitorEmergency = () => {
  ...
  const fetchUnhandledAlerts = async () => {
    try {
      const response = await axios.get("http://localhost:3000/api/unhandled-alerts");
  ...
  const handleAlertClick = async (alert) => {
    ...
    try {
      await axios.post("http://localhost:3000/api/update-handled-time", { alert_id: alert.alert_id });
  ...
  const handleCompleteAlert = async (alert_id) => {
    try {
      await axios.post("http://localhost:3000/api/complete-alert", { alert_id });
